...
Nighthawk.prototype.loop = function(a, b, d, c) {
    this.me = b;
    this.camera = a;
    this.game = c;
    this.inputs = d;
...
Nighthawk.prototype.createMenu = function() {
    ...
    this.settingsMenu = {
        bhop: {
            ...
            val: 0,
            html: function() {
                return '<select  onchange="window.cheat.setSetting(\'bhop\', this.value)"><option value="0"' + (0 == a.settingsMenu.bhop.val ? " selected" : "") + '>Disabled</option><option value="1"' +
        ...
        slide: {
            ...
            val: 0,
            html: function() {
                return "<label class='switch'><input type='checkbox'  onclick='window.cheat.setSetting(\"slide\", this.checked)' " + (a.settingsMenu.slide.val ? "checked" : "") + "><span class='slider'></span></label>"
            },
            set: function(b) {
                a.settings.slide = b
            }
        ...
        esp: {
            ...
            val: 0,
            html: function() {
                return "<label class='switch'><input type='checkbox'  onclick='window.cheat.setSetting(\"esp\", this.checked)' " + (a.settingsMenu.esp.val ? "checked" : "") + "><span class='slider'></span></label>"
            },
            set: function(b) {
                a.settings.esp = b
            }
        ...
        autoReload: {
            ...
            val: 0,
            html: function() {
                return "<label class='switch'><input type='checkbox'  onclick='window.cheat.setSetting(\"autoReload\", this.checked)' " + (a.settingsMenu.autoReload.val ? "checked" : "") +
        ...
        autoAimRange: {
            ...
            val: "Default",
            html: function() {
                var b = "<select  onchange=\"window.cheat.setSetting('autoAimRange', this.value)\">",
                    d = a.getAimRange("all"),
                    c;
...
Nighthawk.prototype.setupSettings = function() {
    for (var a in this.settingsMenu)
        if (this.settingsMenu[a].set) {
            var b = this.getSavedVal("nh_hack_" + a);
...
Nighthawk.prototype.getAimRange = function(a) {
    var b = "Default 100 150 200 250 300 350 400 450 500 750 1000".split(" ");
    return "all" == a ? b : b[a]
};
Nighthawk.prototype.setSetting = function(a, b) {
    ...
    this.saveVal("nh_hack_" + a, b)
};
Nighthawk.prototype.keyDown = function(a) {
...
Nighthawk.prototype.chatMessage = function(a, b, d) {
    0 == $("#haxMsg").length && $("<div>", {
        id: "haxMsg",
        css: {
            "z-index": 999,
...
Nighthawk.prototype.getDist = function(a, b) {
    var d = a.x - b.x,
        c = a.y - b.y;
    a = a.z - b.z;
    return Math.sqrt(d * d + c * c + a * a)
};
Nighthawk.prototype.autoShot = function(a) {
...
Nighthawk.prototype.aimReset = function() {
...
Nighthawk.prototype.aimDelay = function() {
    var a = this.weapons[this.me.weapon.name];
    return a && a.d ? a.d : this.me.weapon.rate - 50
};
Nighthawk.prototype.aimCompensator = function(a) {
...
Nighthawk.prototype.aimbot = function() {
    var a = this,
        b = !1,
        d = window.targets;
    try {
...
Nighthawk.prototype.roundN = function(a, b) {
    return parseFloat(Math.round(a * Math.pow(10, b)) / Math.pow(10, b)).toFixed(b)
};
window.cheat = new Nighthawk;
"1.2" !== document.querySelector("meta[name*=nh_version]").content && (alert("[NEW UPDATE] Download the new Nighthawk update now!"), location.href = "https://raw.githubusercontent.com/richardletshacks/nighthawk-hook/master/script.user.js");
0 > document.querySelector("script[src*='js/____']").src.indexOf("xxfBl") && alert("[UPDATING] Please wait for cheat update.");
